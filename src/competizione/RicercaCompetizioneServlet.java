...
@WebServlet("/RicercaCompetizioneServlet")
public class RicercaCompetizioneServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	static CompetizioniModel modello_competizioni = new CompetizioniModelDS();
	public RicercaCompetizioneServlet() {
		super();
	}
		...
		try {
			if (action != null) {
				if (action.equalsIgnoreCase("search-comp")) {
					...
					request.getSession().setAttribute("lista_competizioni", modello_competizioni.doSearch(nome));
				}
			}
